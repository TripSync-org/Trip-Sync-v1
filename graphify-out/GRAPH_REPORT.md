...
## Corpus Check
- 111 files · ~253,672 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 449 nodes · 539 edges · 21 communities detected
...
- `handler()` --calls--> `send()`  [INFERRED]
  frontend\api\proxy.js → mobile\src\screens\SupportChatScreen.tsx
...
Cohesion: 0.06
Nodes (22): apiFetch(), apiUrl(), isAbortLikeError(), readApiErrorMessage(), formatShortDate(), formatTime12h(), setTimeOnDate(), attachCoupon() (+14 more)
...
Cohesion: 0.09
Nodes (20): App(), handler(), post(), aggregateRevenueFromRows(), computeOrganizerRevenue(), normalizeRevenueDateRange(), num(), registerLiveTripMapRoutes() (+12 more)

### Community 4 - "Community 4"
Cohesion: 0.1
Nodes (22): handler(), handler(), parseBody(), handler(), parseBody(), handler(), parseBody(), canAssignRole() (+14 more)

### Community 5 - "Community 5"
Cohesion: 0.1
Nodes (15): navigateToRootStack(), exportRevenuePdf(), goStack(), openCreate(), saveCoupon(), buildPayoutHistoryHtml(), buildRevenueSummaryHtml(), esc() (+7 more)

### Community 6 - "Community 6"
Cohesion: 0.15
Nodes (9): handler(), handler(), send(), broadcastRaiseHand(), sortParticipants(), useParticipants(), useVoiceChannel(), normalizeRole() (+1 more)
...
## Knowledge Gaps
- **Thin community `Community 7`** (16 nodes): `voiceManagerStub.ts`, `StubVoiceManager`, `.callRider()`, `.constructor()`, `.getConnectedRiders()`, `.getMuted()`, `.handleAnswer()`, `.handleIceCandidate()`, `.handleOffer()`, `.removeRider()`, `.setBlocked()`, `.setMuted()`, `.setRemoteMuted()`, `.setVoiceMode()`, `.start()`, `.stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 9`** (16 nodes): `voiceManagerImpl.ts`, `VoiceManager`, `.callRider()`, `.constructor()`, `.createPeer()`, `.getConnectedRiders()`, `.getMuted()`, `.handleAnswer()`, `.handleIceCandidate()`, `.handleOffer()`, `.setBlocked()`, `.setMuted()`, `.setRemoteMuted()`, `.setVoiceMode()`, `.start()`, `.stop()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 11`** (9 nodes): `realtime.js`, `emitCheckpointsUpdated()`, `emitMapPinRequestedToStaff()`, `emitMapPinReviewedToUser()`, `emitToPrivileged()`, `getTripRiders()`, `haversineMeters()`, `removeRiderLocation()`, `upsertRiderLocation()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 13`** (8 nodes): `MainApplication.kt`, `getJSMainModuleName()`, `getPackages()`, `getUseDeveloperSupport()`, `MainApplication`, `.onConfigurationChanged()`, `.onCreate()`, `MainApplication.kt`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 15`** (7 nodes): `MainActivity.kt`, `MainActivity`, `.createReactActivityDelegate()`, `.getMainComponentName()`, `.invokeDefaultOnBackPressed()`, `.onCreate()`, `MainActivity.kt`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 16`** (6 nodes): `formatDistance()`, `getRouteSegmentMidpoints()`, `haversineDistance()`, `haversineMeters()`, `insertCheckpointByDistance()`, `checkpointUtils.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 18`** (4 nodes): `LiveTripMapNoProvider.tsx`, `joinTripRoom()`, `onConnect()`, `readLiveMapStoredTheme()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (3 nodes): `ParticipantRow.jsx`, `labelForRole()`, `ParticipantRow()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (3 nodes): `OnboardingScreen.tsx`, `finishOnboarding()`, `onNext()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `cleanup()` connect `Community 8` to `Community 0`?**
  _High betweenness centrality (0.159) - this node is a cross-community bridge._
- **Why does `catch()` connect `Community 0` to `Community 1`, `Community 10`?**
